...
const Home = () => {
  ...
  useEffect(() => {
    // Fetch all games from the API endpoint
  ...
  const getGameById = (id) => {
    ...
    axios.get(`http://localhost:8080/games/${id}`)
      .then((response) => {
        ...
        // Handle the specific game data as needed
      ...
      <div className="grid grid-cols-2 gap-4 w-100 mb-[60px]">
        {games && games.map((game) => (
          <div key={game.id} className="border p-4">
            ...
            <button onClick={() => getGameById(game.id)} className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-2">
              View Game by ID
            </button>
          </div>
        ))}
